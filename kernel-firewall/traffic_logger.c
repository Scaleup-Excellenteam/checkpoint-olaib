...
#define HOOK_MSG_SUCCESS "logger traffic success successfully\n"
#define HOOK_MSG_FAIL "logger traffic failed\n")
...
static int __init ftraffic_logger_init(void);
...
static unsigned int log_incoming_traffic(void* priv, struct sk_buff* skb, const struct nf_hook_state* state)
{
    ...
    struct icmphdr* tcph; // Tcp header struct
    if (!skb) return NF_ACCEPT; // if no packet, accept it

    iph = ip_hdr(skb); // get IP header from packet

    if (iph->protocol == IPPROTO_TCP) // if protocol is ICMP
    {
        tcph = icmp_hdr(skb); // get ICMP header from packet
...
static int __init firewall_init(void)
{
    ...
    HOOKS_OPTIONS.PF = NFPROTP.IPV4; // set protocol family
